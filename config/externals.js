...
module.exports = function nodeExternals(options) {
  ...
  var modulesFromAnywhere = !!options.modulesFromAnywhere
  ...
  if (modulesFromFile) {
    nodeModules = readFromPackageJson()
  } else {
  ...
  return function (context, request, callback) {
    ...
    var isIncluded = contains(nodeModules, moduleName)

    if (modulesFromAnywhere) {
      isIncluded = true
    }

    if (isIncluded && !containsPattern(whitelist, request)) {
